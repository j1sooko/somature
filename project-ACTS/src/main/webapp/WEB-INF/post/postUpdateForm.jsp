...
<body>
	...
	<form name="form" method="POST" action="<c:url value='/post/update' />"
		...
		<!-- seller or buyer -->
		<label for="exampleFormControlInput1" class="form-label">acts
			타입</label>
		<div class="form-check">
			<input class="form-check-input" type="radio" name="pType"
				id="flexRadioDefault1" value="s" checked> <label
				class="form-check-label"> seller </label>
		</div>
		<div class="form-check">
			<input class="form-check-input" type="radio" name="pType"
		...
		<!-- 게시글 status -->
		<label for="exampleFormControlInput1" class="form-label">상태</label> <br>
		...
		<!-- 게시글 카테고리 -->
		<br> <label for="exampleFormControlInput1" class="form-label">카테고리</label>
		<br> <select name="categoryId" style="width: 240">
		...
		<!-- 가격 -->
		<div class="mb-3">
			<label for="exampleFormControlInput1" class="form-label">가격</label> <br>
			<input name="price" class="form-control  input-sm"
				id="exampleFormControlInput1" aria-label=".form-control-lg example"
				value="${post.price}">원
		</div>
		...
		<div>
			<div>
				<textarea name="description" id="content_textarea" cols="100"
					rows="20">${post.desc}</textarea>
			</div>
		</div>

		<!--  이미지 업로드 -->
		<input type="file" id="image" accept="image/*"
			onchange="setThumbnail(event);" name="image" /> <br><%=request.getRealPath("/")%>
		...
		<br> <br> <input type="button" value="등록"
			onClick="postCreate()" class="btn btn-primary">
	</form>
